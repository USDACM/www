...
if ( is_multisite() ){
	...
	$user_can_edit = ( $tmp_blog_id && current_user_can_for_blog($tmp_blog_id, 'edit_posts') ) || current_user_can('edit_posts');
}
else {
	$tmp_user_url = site_url() . '/';
...
<?php if ( $user_can_edit ): ?>
	...
	<br />
	<a href="<?php echo $tmp_user_url; ?>wp-admin/profile.php" title="<?php _e('Edit your profile',TEMPLATE_DOMAIN) ?>"><?php _e('Edit your profile',TEMPLATE_DOMAIN); ?></a>
<?php else: ?>
	<?php _e("Welcome back,",TEMPLATE_DOMAIN); ?> <?php echo $user_identity; ?>
	<br />
	<?php $mywp_version = get_bloginfo('version'); if ($mywp_version >= '2.7') { ?> <a href="<?php echo wp_logout_url( get_site_url() ); ?>"><?php _e('Log out &raquo;',TEMPLATE_DOMAIN); ?></a> <?php } else { ?> <a href="<?php echo site_url('/wp-login.php?action=logout', 'login'); ?>" title="<?php _e("Log out of this account",TEMPLATE_DOMAIN); ?>"><?php _e('Log out &raquo;',TEMPLATE_DOMAIN); ?></a> <?php } ?>
<?php endif ?>
</p>

<?php if ( $user_can_edit ): ?>
	<p><?php _e("Welcome back,",TEMPLATE_DOMAIN); ?> <?php echo $user_identity; ?>, <?php _e("use the links above to get started or you can",TEMPLATE_DOMAIN); ?> <?php $mywp_version = get_bloginfo('version'); if ($mywp_version >= '2.7') { ?> <a href="<?php echo wp_logout_url( get_site_url() ); ?>"><?php _e('Log out &raquo;',TEMPLATE_DOMAIN); ?></a> <?php } else { ?> <a href="<?php echo site_url('/wp-login.php?action=logout', 'login'); ?>" title="<?php _e("Log out of this account",TEMPLATE_DOMAIN); ?>"><?php _e('Log out &raquo;',TEMPLATE_DOMAIN); ?></a> <?php } ?>.</p>
<?php endif ?>

<?php if ( get_posts('post_type=page&pagename=forums') ): ?>
